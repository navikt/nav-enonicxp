...
const doArrayByPath = (arrayFn) =>
    R.curry((objPath, data, object) =>
        R.pipe(
...
const setByPath = R.curry((objPath, data, object) => R.set(lensStringPath(objPath), data, object));
...
const setEntryForPath = recurry((entryPath) =>
    setByPath(`entry.${pathWithoutExt(entryPath)}`, `./${entryPath}`)
);
// setEntriesForPath :: Array -> Object -> Object
const setEntriesForPath = R.curry((entries, config) =>
    R.reduce((cfg, entry) => setEntryForPath(entry, cfg), config, entries)
);
