...
export const assertLocalTargetConfiguration = (sandboxPath) => {
    ...
    if (properties.env !== 'localhost' || properties.curatedImportEnabled !== 'true') {
        throw new Error(
            'Target must explicitly configure env=localhost and curatedImportEnabled=true'
        );
    }
...
export const assertLocalTargetProcess = (
    sandbox,
    { homeDirectory = homedir(), runCommand = execFileSync } = {}
) => {
