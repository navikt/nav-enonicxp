...
import { isDeferredRelocationError } from '../import-curated-export.mjs';
import { getImportOptions } from '../sandbox-import.mjs';
...
test('the public create/update command loads without removed CLI modules', () => {
    const result = spawnSync(
        process.execPath,
        [fileURLToPath(new URL('../sandbox-import.mjs', import.meta.url))],
        { encoding: 'utf8', env: {} }
    );
...
test('streams import progress before the child script finishes', async (t) => {
    ...
    const runner = new URL('../sandbox-import.mjs', import.meta.url).href;
    const child = spawn(
        process.execPath,
        [
            '--input-type=module',
