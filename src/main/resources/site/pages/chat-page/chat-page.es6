...
function handleGet(req) {
    ...
    const assets = [
        '<link rel="apple-touch-icon" href="' +
            libs.portal.assetUrl({ path: 'img/navno/logo.png' }) +
            '" />',
        '<link rel="shortcut icon" type="image/x-icon" href="' +
            libs.portal.assetUrl({ path: 'img/navno/favicon.ico' }) +
            '" />',
        '<link rel="stylesheet" href="https://chat.puzzel.com/Content/Client/css/intelecom-light.css" />',
        '<link rel="stylesheet" href="' +
            libs.portal.assetUrl({ path: 'styles/navno.css' }) +
            '" />',
        '<link rel="stylesheet" href="' +
            libs.portal.assetUrl({ path: 'styles/chat-puzzel.css' }) +
            '" />',
        '<script id="navno-props" src="' +
            libs.portal.assetUrl({ path: 'js/navno-page.js' }) +
    ...
    const scripts = [
        '<script id="chatProps" ' +
            'customerKey="' +
            content.data.customerKey +
            '" queueKey="' +
            content.data.queueKey +
            '" timeId="' +
            content.data.timeId +
            '" src="' +
            libs.portal.assetUrl({ path: 'js/chat-puzzel.js' }) +
            '"></script>',
    ];
    const model = {
