$(function initAddressOptionsForm() {
    ...
    function initAutocomplete() {
        changenavdepartment1(); // TODO: revurder
    ...
    function handleFormSubmit() {
        $("#address-options-form").submit(function (event) {
            ...
            if ($("input[name='hasfrontpage'][value='true']:checked").length > 0) {
                return true;
            }
            var isFormValid = false;

            var validCheckedInput = $("input[type='radio']:checked").filter(function () {
    ...
    function validateInputFields(inputElements, errorPanel) {
        ...
        if (!isInputValid) {
            errorPanel.addClass("showing").show();
        }
        else {
            errorPanel.remove();
        }
