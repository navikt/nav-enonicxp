...
export const getAttachmentText = (attachment: GuillotineAttachment, maxSize = maxSizeDefault) => {
    if (!attachment) {
        logger.warning(`No attachment object was provided`);
        return null;
    }
    ...
    if (!id || !name) {
        logger.warning(`Id or name for attachment not found - got id ${id} and name ${name}`);
        return null;
    }

    if (size > maxSize) {
        logger.warning(`Max size exceeded for attachment - id ${id} - name ${name}`);
        return null;
    }

    const attachmentStream = contentLib.getAttachmentStream({ key: id, name: name });
    if (!attachmentStream) {
        logger.warning(`No attachment stream found for ${id} ${name}`);
        return null;
    }

    const attachmentText = ioLib.readText(attachmentStream);
    if (!attachmentText) {
        logger.warning(`No attachment text found for ${id} ${name}`);
        return null;
    }
