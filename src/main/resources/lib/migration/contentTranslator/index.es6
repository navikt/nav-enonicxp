...
function transSidebeskrivelse (indexConfigurations, socket) {
    ...
    ar.forEach(function (id, index) {
        ...
        var sidebeskrivelse = contentLib.get({
            key: id,
        });
        ...
        moveFromContentSiteToContent(sideHome.data.sectionContents, {
            path: sideHome._path, id: sideHome._id,
        });
    });
...
function transMinSections (id, socket) {
    ...
    r.forEach(function (value, index) {
        ...
        if (content) {
            ...
            moveFromContentSiteToContent(content.data.tableContents, {
                path: content._path, id: content._id,
            });
...
function transcms2xpPage (indexConfig, socket) {
                ...
                if (!ret[stripContentType(article.type)]) {
                    ...
                    repo.modify({
                        ...
                        editor: function () {
                            ...
                            if (parameters) {
                                cms2xp.data.parameters = parameters;
                            }
    ...
    for (var articleId in articles) {
        ...
        refInfo.pathsExtd.forEach(function (ref) {
            ...
            tools.modify(contentLib.get({
                key: ref.id,
            }), cms2xpPage._id, articleId);
        });
