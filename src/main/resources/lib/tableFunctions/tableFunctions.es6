...
function filterAttributes(element) {
    ...
    if (element.attribute && element.attribute.class) {
        element.attribute.class = element.attribute.class.reduce(function(p, e) {
            if (p.indexOf(e) && !e.startsWith('stat')) {
                p.push(e);
            }
            return p;
        }, []);
...
function checkIfStatGroupHeadOrStatGroupRestAndChange(element) {
    var isStatGroup = function(e) {
        return (
            e.tag === 'td' &&
            e.attribute.class &&
            e.attribute.class.reduce(function(p, e) {
                return p || e.indexOf('statGroup') > -1;
            }, false)
        );
    };
...
function changeTh(e) {
    return doIf(
        isTh(e),
        mix(setAttribute('scope', 'col'), function(e) {
...
function changeFirstTd(element) {
    ...
    element.children = recurse(
        mix(doIfFirstChild, checkIfStatGroupHeadOrStatGroupRestAndChange),
        'children',
        element
    );
    return element;
...
function changeFirstFirstTd(element) {
    ...
    var doIfFirstChild = doIf(
        isFirstChild,
        mix(setExtendedNavTextClass, setScopeRow),
        setStrongOnElement
    );
...
function changeTbodyChildren(element) {
    ...
    var changeTrElementAndIsFirstChild = doIf(
        isFirstChild,
        mix(alterTrElement, changeFirstFirstTd),
        changeTrElement
    );
...
function changeH1Tag(element) {
    return doIf(
        element.tag === 'h1',
        changeTagValue('tag', 'h1 class="h4"'),
        returnElement,
        element
    );
}
...
function unmap(array, string) {
    ...
    var rattributes = split.reduce(function(t, el) {
        // Reduce the rest of attributes to an object with
    ...
    while (next && !next.startsWith('</' + tag + '>')) {
        // If it is not the closing tag, then it is an element
...
function m(o, skipFirst) {
    // Builds the JSON to html again
    var s = '';
    if (!skipFirst) {
        s = '<' + o.tag;
