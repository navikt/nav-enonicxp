...
exports.get = function(req) {
    var ls = repo.get("88a5a9f6-bdc6-4617-8699-f18a1a200135");
    var routeBucket = [];
    var contentBucket = [];
    var mediaBucket = [];
    var mailtoBucket = [];
...
function tryToFixRoute(routes) {
    routes.forEach(function (route) {
      ...
      var id = contentLib.get({
          key: '/www.nav.no' + route.address.replace(/\+/g, '-')
      });
      ...
      if (id) {
          contentLib.modify({
              ...
              branch: "master",
              editor: function (c) {
                  if (c.data.text) c.data.text = c.data.text.replace(route.address, 'content://' + id._id);
                  else trans.logBeautify(c);
                  return c;
              }
          })
      }
