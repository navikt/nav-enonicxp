...
function changeContrast() {
	...
	if (bodyClass.indexOf('contrast') == -1) {
		bodyClass += ' contrast';
      		if (bodyClass.indexOf('normal') != -1) { // Force largest font size
      		    bodyClass = bodyClass.replace('normal', 'largest');
      		}
      		else if (bodyClass.indexOf('largest') == -1) { // font sisze is large. Force largest.
      		    bodyClass = bodyClass.replace('large', 'largest');
      		}
		body.setAttribute(msIEversion < 8  ? 'className' : 'class', bodyClass);
	} else 
	   { // Turn off contrast and reset fontsize
		...
		if (bodyClass.indexOf('largest') != -1) { // Force normal font size
      		    bodyClass = bodyClass.replace('largest', 'normal');
      		}
      		else if (bodyClass.indexOf('large') != -1) {
      		    bodyClass = bodyClass.replace('large', 'normal');
      		}
		body.setAttribute(msIEversion < 8  ? 'className' : 'class', bodyClass);
	}	
}
